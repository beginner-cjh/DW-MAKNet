...
def rearrange_center_blocks(x: torch.Tensor, kernel: int) -> torch.Tensor:
    ...
    output1 = torch.cat(flattened, dim=-1)  # (B, C, total_L) #从左到右

    for i, block in enumerate(blocks_ud):
        # 计算当前分块的展平长度
        flattened[i] = block.transpose(2, 3).contiguous().view(B, C, -1)
    output2 = torch.cat(flattened, dim=-1)  # 从上到下

    output3 = torch.flip(output1,dims=[-1])  # 从右到左
    output4 = torch.flip(output2, dims=[-1])  # 从下到上
...
class DRCMKNetEncoder(nn.Module):
    """ Encoder.
            Args:
    ...
    def __init__(self, patch_size=2, depths=[2, 4, 4, 2], feat_size=[24, 48, 96, 192, 384, 768],
        ...
        for i_layer in range(self.num_layers):
            layer = DRCMKBlock(dim=dims[i_layer],
        ...
        for i_layer in range(self.num_layers):
            layer = DRCMKBlock(dim=dims[i_layer+2],
...
class DRCMKNetDecoder(nn.Module):
    """ Decoder.
                Args:
...
class DRCMKNet(nn.Module):
    def __init__(self, encoder_args, decoder_args):
        super().__init__()
        self.encoder = DRCMKNetEncoder(**encoder_args)
        self.decoder = DRCMKNetDecoder(**decoder_args)
...
def get_drcmknet_from_plans(
    ...
    model = DRCMKNet(encoder_args, decoder_args)
